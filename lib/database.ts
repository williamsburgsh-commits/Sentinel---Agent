...
export async function createSentinel(
  ...
  try {
    // First, deactivate all existing sentinels for this user
    await deactivateAllSentinels(userId);
...
export async function getSentinels(userId: string): Promise<Sentinel[]> {
  const supabase = createBrowserClient();
...
export async function deactivateAllSentinels(userId: string): Promise<boolean> {
  const supabase = createBrowserClient();
